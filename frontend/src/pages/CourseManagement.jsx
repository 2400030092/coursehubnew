...
import { deleteCourse as deleteCourseApi, fetchInstructorCourses } from '../services/courseApi';
...
export const CourseManagement = () => {
  ...
  const fetchCourses = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetchInstructorCourses(user.id);
      setCourses(response);
    } catch (error) {
  ...
  const handleDeleteCourse = async (courseId) => {
    if (window.confirm('Are you sure you want to delete this course?')) {
      try {
        await deleteCourseApi(courseId, user.id);
        setCourses(courses.filter(course => course.id !== courseId));
        toast.success('Course deleted successfully');
      } catch (error) {
        toast.error(error.message || 'Unable to delete this course');
      }
    }
  };
